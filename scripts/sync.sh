...
# --------------------------------------------------------------------------
# Portão de testes — código quebrado não viaja
# --------------------------------------------------------------------------
#
# Este repositório não é só um vault de notas: `agente_drop/` é um projeto
# Python com 156 testes que sobe o drop da marca para a Nuvemshop. Uma
# sincronia que empurra commit quebrado leva o defeito para a outra máquina
# e para o main, sem ninguém revisar — que é justamente o preço de commitar
# automaticamente.
#
# Regra: se os commits prestes a subir tocam código, os testes rodam antes do
# push. Falharam, o trabalho FICA COMMITADO aqui — nada se perde — mas não
# sobe. Nota, plano e planilha passam direto, sem teste, que é o caso comum.

CAMINHOS_DE_CODIGO=(agente_drop testes ferramentas .github)

encontrar_python() {
  local nome
  # `python3` primeiro no Mac e no Linux. No Windows ele costuma ser o stub da
  # Microsoft Store, que não executa nada — por isso `py` entra na lista.
  for nome in python3 python py; do
...
portao_de_testes() {
  local branch="$1"
  local marca="${STATE_DIR}/TESTES-FALHARAM.txt"
  local py tocou

  tocou="$(git_v diff --name-only "origin/${branch}...HEAD" -- "${CAMINHOS_DE_CODIGO[@]}" 2>/dev/null || true)"
  if [ -z "${tocou}" ]; then
    rm -f "${marca}"
    return 0
  fi
  ...
  if ! py="$(encontrar_python)"; then
    log AVISO "os commits mexem em código, mas não achei python neste PC; enviando sem testar"
    return 0
  fi

  log INFO "os commits mexem em código — rodando os testes antes de enviar (${py})"
  # PYTHONDONTWRITEBYTECODE: sem isto o próprio portão sujaria a árvore com
  # __pycache__/, e a sincronia commitaria esse lixo no ciclo seguinte — a cada
  # 15 minutos, para sempre. Um passo de verificação não pode alterar a árvore
  # que ele está prestes a sincronizar.
  if ( cd "${VAULT_DIR}" && PYTHONDONTWRITEBYTECODE=1 "${py}" -m unittest discover -s testes -t . >>"${LOG_FILE}" 2>&1 ); then
    log INFO "testes passaram"
    rm -f "${marca}"
    return 0
  fi

  log ERRO "os testes falharam — nada foi enviado; seu trabalho está commitado aqui e não se perdeu"
  mkdir -p "${STATE_DIR}"
  {
    printf 'Sincronia segurou o envio em %s, no PC %s.\n\n' \
      "$(date '+%Y-%m-%d %H:%M:%S')" "${HOSTNAME_SHORT}"
    printf 'Os testes do agente de drop falharam. Seu trabalho está salvo no commit\n'
    printf 'local — nada foi perdido — mas não subiu, para não levar o defeito para\n'
    printf 'a outra máquina e para o main.\n\n'
    printf 'Veja o erro:\n  %s -m unittest discover -s testes -t .\n\n' "${py}"
    printf 'Corrija e o próximo ciclo envia sozinho. Detalhes em:\n  %s\n' "${LOG_FILE}"
  } >"${marca}"

  # stderr mesmo em modo silencioso, para o agendador registrar.
  printf 'ATENÇÃO: os testes falharam; a sincronia segurou o envio. Veja %s\n' "${marca}" >&2

  # Notificação do sistema é um extra: se falhar, o log e o arquivo acima
  # continuam valendo. Nunca deixar isso derrubar a sincronia.
  if command -v osascript >/dev/null 2>&1; then
    osascript -e 'display notification "Testes falharam. Trabalho salvo local, nao enviado." with title "BVBA — envio segurado"' >/dev/null 2>&1 || true
  elif command -v notify-send >/dev/null 2>&1; then
    notify-send 'BVBA — envio segurado' 'Testes falharam. Trabalho salvo local, nao enviado.' >/dev/null 2>&1 || true
  fi
  return 1
}
...
main() {
  ...
  while [ "${round}" -le "${SYNC_ROUNDS}" ]; do
    ...
    # Portão de testes antes do push, já com o remoto integrado: o que os
    # testes avaliam é exatamente a árvore que iria para a outra máquina.
    if ! portao_de_testes "${branch}"; then
      exit 72
    fi
