...
### Código quebrado não viaja

Este repositório não é só um vault de notas: `agente_drop/` é um projeto Python
com **156 testes** que sobe o drop da marca para a Nuvemshop. Como a sincronia
commita e envia sozinha, sem ninguém revisar, um defeito viajaria para a outra
máquina e para o `main` no piloto automático.

Por isso existe um portão, e ele só acorda quando precisa:

| O que você mudou | O que acontece |
|---|---|
| Nota, plano, pesquisa, planilha | envia direto — nenhum teste roda |
| `agente_drop/` · `testes/` · `ferramentas/` · `.github/` | roda os 156 testes **antes** do envio |

Se os testes falharem, o ciclo para antes do `git push`:

- **o seu trabalho fica commitado nesta máquina** — nada é descartado, nada
  volta atrás;
- **nada sobe**, então a outra máquina e o `main` seguem sãos;
- aparece um `.sync/TESTES-FALHARAM.txt` dizendo o que rodar para ver o erro;
- corrigido o defeito, o ciclo seguinte envia tudo sozinho, inclusive o que
  ficou represado.

O portão roda com `PYTHONDONTWRITEBYTECODE=1` de propósito: sem isso ele
deixaria `__pycache__/` para trás e a sincronia commitaria esse lixo no ciclo
seguinte — a cada 15 minutos, para sempre. Um passo de verificação não pode
sujar a árvore que está prestes a sincronizar.

Se a máquina não tiver Python, o portão avisa no log e deixa passar, em vez de
travar a sincronia das notas por causa de uma ferramenta ausente.
